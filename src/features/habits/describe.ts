 ...
 * `showAvoid: false` omite «A evitar» donde ya lo dice el sobretítulo (detalle del hábito).
 */
export function describeHabit(
  habit: Pick<Habit, 'kind' | 'target' | 'unit' | 'frequency' | 'timeOfDay'>,
  weekStartsOn: Weekday,
  categoryName?: string | null,
  { showAvoid = true }: { showAvoid?: boolean } = {},
): string {
  ...
  if (habit.kind === 'avoid' && showAvoid) parts.push('A evitar');
