...
- **Selector de color:** se presenta en `HABIT_COLOR_DISPLAY_ORDER` (`ui/HabitMarks.tsx`, por tono y el gris al final), **no** en el orden de `HABIT_COLORS`, que fija el color por defecto de un hábito nuevo (`palette.test.ts` comprueba que es una permutación).
- **Selector de «Momento del día» (mismo patrón que el de color, no lo unifiques):** el formulario de hábito lo presenta en `TIME_OF_DAY_DISPLAY_ORDER` (`lib/format.ts`), con `any` («Cualquiera») primero, igual que Tipo y Frecuencia. **No** es `TIME_OF_DAY_ORDER` (`domain/today.ts`), que agrupa la pantalla Hoy y va Mañana, Tarde, Noche y Cualquier momento al final — ese orden depende del reloj (rachas y frecuencias) y no cambia. `format.test.ts` comprueba que `TIME_OF_DAY_DISPLAY_ORDER` es una permutación de `TIME_OF_DAY_ORDER` y que no coinciden, para que nadie los vuelva a fundir en una sola constante. `flex flex-wrap` de celdas de 44 px sin `gap`: con 13 colores caben en una fila en escritorio y 7+6 en móvil de 360 y 375 px (con `gap-1` a 360 salía 6+6+1; a 320 sigue siendo 6+6+1). Si añades colores, vuelve a medirlo.
...
- **Heatmap:** el año se dibuja entero; los días anteriores a la creación son celdas `before` (contorno tenue, sin botón). Celdas de 24 px en móvil (`size-6`) y de 12 px desde `lg`. La columna de días es `sticky`.
  - **Bug de layout (corregido, 2026-09-22): las etiquetas de mes se solapaban en móvil** ("sepoct", "mayjun" pegados). No era que el texto no cupiera: medido en el DOM real, hasta 26 de las 53 columnas de semana renderizaban a 1,6 px en vez de 24 px, pese a llevar `size-6` en el `<th>`/`<td>`. La tabla no tiene `table-layout: fixed` ni `colgroup`, así que el navegador trata el `width` de cada celda como una simple referencia: cuando el ancho natural de la tabla (~1487 px en móvil) no cabe en el contenedor, reparte el hueco disponible dando prioridad a las columnas con contenido "real" (el `<button>` de un día con historia) y aplasta las que solo llevan un `<span>` decorativo (celdas `before`/`outside`), muy por debajo de su `size-6`. En escritorio nunca pasa porque a 12 px por celda el año entero (~825 px) sí cabe sin comprimir nada. Como la etiqueta de mes es un `<span>` en posición absoluta sin recorte, con la columna aplastada a 1,6 px el hueco hasta la siguiente etiqueta cae muy por debajo del ancho del texto y los meses se solapan. Arreglo: `CELL = 'size-6 min-w-6 lg:size-3 lg:min-w-3'` (`Heatmap.tsx`) — un `min-width` igual al ancho evita que cualquier columna se comprima, sin tocar `heatmapModel.ts` (el cálculo de qué semana lleva la etiqueta ya era correcto) ni la estructura de la tabla. Probado que `table-layout: fixed` + `colgroup` con anchos en línea **no** lo arregla por sí solo en este componente; hace falta `min-width`.
...
- **Registro del día sin hábitos:** ánimo, energía y nota no pertenecen a ningún hábito. `TodayScreen` pinta `DayLogPanel` siempre (salvo Onboarding), debajo del estado vacío. `deleteHabit`/`archiveHabit` nunca tocan `dayLogs` (`db/repos.test.ts`), la copia los incluye sin hábitos (`db/dataset.test.ts`) y flechas y límite retroactivo funcionan igual (`TodayScreen.test.tsx`). Solo «Borrar todo» los elimina.
  - **Onboarding y registro del día (cerrado):** `TodayScreen` enseña el onboarding solo con `!onboardingDone && !hasHabits && dayLogs.length === 0` (`useDayLogs()`; mientras cargan, no pinta nada). Quien ya guardó ánimo, energía o nota no está empezando, así que ve el estado vacío con «Cómo fue el día». Los `dayLog` vacíos se borran al guardarse, por lo que «existe alguno» equivale a «hay datos». «Borrar todo» vacía también `dayLogs` y `settings`, así que reactiva el onboarding como antes (test y comprobado con Playwright). Efecto secundario: importar una copia con dayLogs pero sin hábitos y con `onboardingDone: false` tampoco lo muestra; es coherente con el criterio. Tests en `TodayScreen.test.tsx` › «sin hábitos».
...
- **Desplazamiento entre pantallas** (`useRouteScroll` en `app/Layout.tsx`, memoria en `app/scrollMemory.ts`, tests en `App.test.tsx`): cambiar de **ruta** (`pathname`) empieza arriba con `scrollTo({ top: 0, behavior: 'instant' })`; un cambio solo de la URL de búsqueda (`?dia=`, rango) no mueve nada; atrás/adelante (`POP`) recupera la posición de esa entrada del historial (`location.key`, guardada en `sessionStorage` bajo `tracker:scroll` como `[ruta, y]`, últimas 50). `history.scrollRestoration` se pone en `manual`.
  - **Causa del bug (2026-09-26, medida en móvil):** no había ningún manejo de desplazamiento (`BrowserRouter` a secas). Una navegación del cliente no recarga la página, así que el `scrollY` de Ajustes (2152 px) se heredaba y el navegador lo recortaba al alto de `/privacidad` (988 − 812 = 176 px, abajo del todo). En escritorio Ajustes no llega tan abajo. **El foco no era la causa:** ya usaba `preventScroll: true`, y con `focus()` neutralizado seguía saliendo 176. Que Hábitos → detalle o Hoy → detalle «funcionaran» era casualidad: el documento se encoge un instante al montarse la pantalla nueva y el navegador recorta a 0, según la temporización.
  - **La restauración nativa tampoco funcionaba** al volver (Privacidad → atrás dejaba Ajustes en 0): pierde la carrera contra las pantallas `lazy()` y contra los datos de `liveQuery`, que aún no dan alto al documento. Por eso es a mano y **reintenta hasta 1,5 s** hasta que la pantalla tiene altura suficiente; una rueda, un toque o una tecla la cancelan.
  - **Trampa: no guardes la posición escuchando `scroll`.** Al montarse la pantalla nueva el documento se encoge y el navegador recorta el desplazamiento (a 0 incluso); ese evento llegaba a pisar lo guardado antes de que se limpiara el listener (falló ~1 de cada 25 veces, solo Hoy → detalle; una heurística por altura no lo arregló porque la altura previa quedaba obsoleta). Se guarda **al iniciar** la navegación (`click`, `keydown`, `popstate`, `pagehide`, en captura), con el valor real. Mientras se restaura, esos avisos no guardan (el `scrollY` es parcial).
  - **La clave `default`** la reutiliza la primera entrada de cada carga completa, por eso se guarda también la ruta, y en la carga inicial solo se restaura si el navegador la marca como recarga o vuelta atrás (`performance.getEntriesByType('navigation')`).
  - **Verificado con Playwright** (375×812 y 1280×500, claro y oscuro): Ajustes → Privacidad, Hábitos → detalle, Hoy → detalle, pestañas, crear y editar hábito de vuelta a la lista, todo a `scrollY = 0` con el foco en `#contenido`; atrás recupera la posición (72/72 en un estrés con esperas de 0–900 ms). Cambiar de día en Hoy puede mover el desplazamiento solo porque el otro día es más corto (recorte por altura), no por esta lógica.
...
- **Encabezado del detalle de hábito (decisión consciente, no lo «corrijas»):** la `ColorBar` va pegada al margen y el texto del título queda desplazado a su derecha (unos 47 px en escritorio y móvil respecto al resto de pantallas). El encabezado en sí (`header`) sí coincide en x e y con todas las demás, y el `h1` cae a la misma altura (y = 40 en móvil, 56 en escritorio); solo el texto empieza más adentro. Se decidió así para no romper la regla de filas con marca de color. No saques la barra fuera del margen ni alinees el título con el de otras pantallas.
  - **Reorganizado el 2026-09-22 (propuesta A de tres, elegida por el usuario tras compararlas con capturas):** la barra marca solo la identidad —sobretítulo, nombre y línea de contexto—. El nombre se parte en varias líneas (`wrapTitle`) en vez de cortarse. Por debajo de `lg`, `Editar` y `⋯` bajan a una fila propia **fuera** de la barra; desde `lg` vuelven a la derecha del título. Descripción y aviso de «Archivado» van debajo, también fuera de la barra (`below`). La línea de contexto no repite «A evitar» (`describeHabit(…, { showAvoid: false })`): ya lo dice el sobretítulo. Descartadas: «Editar» dentro del menú `⋯` (a 320 px el nombre ocupaba 4 líneas y editar costaba un toque más) y acciones en la línea del sobretítulo (bajaba el `h1` 28 px respecto a las demás pantallas). Las tres props nuevas de `ScreenHeader` no afectan a nadie más: comprobado con Hoy, Estadísticas, Hábitos, Revisión, Ajustes, formulario nuevo/editar y 404, a 375 y 1280 px en claro y oscuro (32 capturas): el `outerHTML` del `header` y los PNG son idénticos byte a byte antes y después.
  - **Trampa al comparar antes/después:** un `git worktree` con `node_modules` enlazado por symlink no sirve: Vite responde 403 a la fuente (queda fuera de `server.fs.allow`) y las capturas salen con la fuente de reserva. Usa `git stash` en el checkout principal y el mismo puerto (así se comparte la IndexedDB con los datos de ejemplo), y comprueba que `document.fonts` tiene IBM Plex cargada.
